...
Status ProcessConstantValue(QnnModelWrapper& qnn_model_wrapper,
  ...
  if (input.quant_param.has_value()) {
    ORT_RETURN_IF_NOT(input_info.quant_param.params.quantizationEncoding == QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                      "Pad constant value cannot be quantized per-channel");
    ...
    switch (input_info.qnn_data_type) {
      case QNN_DATATYPE_SFIXED_POINT_8: {
        ...
        constant_value = static_cast<float>(utils::Dequantize(input_info.quant_param.params.scaleOffsetEncoding.offset,
                                                              input_info.quant_param.params.scaleOffsetEncoding.scale,
