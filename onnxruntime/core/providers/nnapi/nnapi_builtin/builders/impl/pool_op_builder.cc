...
bool PoolOpBuilder::IsNodeUnitTypeSupported(const NodeUnit& node_unit, const logging::Logger& /*logger*/) const {
...
bool PoolOpBuilder::IsOpSupportedImpl(const GraphViewer& graph_viewer, const NodeUnit& node_unit,
  ...
  if (is_average_pool || op_type == "MaxPool") {
    ...
    if (count_include_pad == 1) {
      LOGS(logger, VERBOSE) << "count_include_pad == 1 is not supported";
      return false;
    }
    ...
    LOGS(logger, VERBOSE) << "PoolOpBuilder, unknown op: " << op_type;
    return false;
  }
  ...
  if (is_average_pool && is_quant_pool) {
    ...
    if (input_scale != output_scale) {
      LOGS(logger, VERBOSE) << "Op [" << op_type << "] name [" << op_name
...
bool PoolOpBuilder::HasSupportedInputOutputsImpl(const GraphViewer& graph_viewer, const NodeUnit& node_unit,
  ...
  if (!is_max_pool && !is_quant_average_pool)
    return BaseOpBuilder::InputIsFloat(node_unit, 0, logger);
