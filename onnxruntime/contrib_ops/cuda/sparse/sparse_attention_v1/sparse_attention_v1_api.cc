...
static std::once_flag load_sparse_attention_sm75_bf16_flag;
...
void load_sparse_attention_bf16(int sm) {
  if (sm == 75) {
    std::call_once(load_sparse_attention_sm75_bf16_flag, load_sparse_attention_bf16_sm75);
  } else {
    assert(sm == 80 || sm == 86 || sm == 89);
...
Status run_sparse_attention_bf16(SparseAttentionParams& params) {
  ...
  if (params.sm == 75) {
    return sparse_attention_bf16_sm75(params, algo_id);
  } else {
