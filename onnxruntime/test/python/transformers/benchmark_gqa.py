...
Install required packages before running this script (For Linux):
...
def plot_prompt_performance(
    ...
    def benchmark(
        batch_size, num_heads, kv_num_heads, sequence_length, head_size, local_window_size, provider, device="cuda"
    ):
        warmup = 15
        repeat = 100
...
def plot_token_performance(
    ...
    configs = [
        triton.testing.Benchmark(
            ...
            x_vals=[2**i for i in range(4, 17) if 2**i + 1 <= max_seq_len] + [max_seq_len - 1],
            line_arg="provider",
            ylabel="ms",
            **algos,
...
def run_performance_test(sm: int):
    ...
    """
    # Note: some models use bf16. We use fp16 for all models since bf16 is not supported in ORT python API.
    configures = [
        ...
        (40, 128, 10, 4096, None, "Phi-3-medium-4k"),
    ]
